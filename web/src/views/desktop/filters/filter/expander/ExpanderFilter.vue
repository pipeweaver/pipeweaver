        ...
        <div class="fields-grid">
          <Field label="Mode" full>
            <DropMenu :values="modeOptions()" :selected="`${getParam('em').value.Int32}`"
                      @valueClicked="setParam('em', $event)"/>
          </Field>
          <Field label="Ratio">
            <NumberInput :min="getParam('er').min" :max="getParam('er').max" :step="0.1"
                         :value="getParam('er').value.Float32"
                         @input="setParam('er', $event)" :allow-empty="false"/>
          </Field>
          <Field label="Knee">
