//! Clients (applications) and their nodes that pipewire told us about but
//! that we didn't create - the audio-producing/consuming counterpart to
//! `unmanaged_devices`. Mirrors that module's "wait until synced and usable,
//! then announce upstream" pattern, but keyed off client state (binary name,
//! running state) rather than port counts.

use super::{Store, TargetType};
