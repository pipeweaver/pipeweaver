...
impl RoutingManagement for PipewireManager {
    async fn routing_set_route(&mut self, source: Ulid, target: Ulid, enabled: bool) -> Result<()> {
        ...
        let error = "[INTERNAL] Broken Target Map";
        let target_id = match target_type {
            NodeType::PhysicalTarget => target,
            NodeType::VirtualTarget => *self.target_map.get(&target).ok_or(anyhow!(error))?,
            _ => bail!("Target is a Source!")
        };
